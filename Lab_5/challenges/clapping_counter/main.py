...
from detection_logic import detect_clap
...
def main():
    ...
    # Setup camera
    # cap = setup_camera()
    cap = setup_camera(raspberry_pi_url="http://cvpi33.local:5000/video")

    fps_calculator = FPSCalculator()
    
    clap_count = 0
    
    print("Clapping Counter Challenge")
    print("Press 'q' to quit")
    print("Press 'r' to reset counter")
    print("Implement detect_clap() in detection_logic.py")
    print("Try clapping to test your implementation")
    ...
    while cap.isOpened():
        ...
        # Detect clap if hands are found
        if results.multi_hand_landmarks and len(results.multi_hand_landmarks) == 2:
            ...
            detection_result = detect_clap(all_hand_landmarks)
            
            # Handle both old format (bool) and new format (tuple)
            if isinstance(detection_result, tuple):
                clap_detected, keypoints_used = detection_result
            else:
                clap_detected = detection_result
                keypoints_used = []
            ...
            # Increment counter if clap detected
            if clap_detected:
                clap_count += 1
        
        # Display counter and status
        draw_counter(image, clap_count, "Claps")
        draw_status(image, clap_detected)
        ...
        if keypoints_used:
            ...
            cv2.putText(image, keypoints_str, (10, 170), 
        ...
        # Show the image
        cv2.imshow('Clapping Counter Challenge', image)
